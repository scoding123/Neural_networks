...
class linear_layer:
    ...
    def forward(self, X):
        ...
        ################################################################################
        X_W_dot = np.dot(X,self.params['W'])
        forward_output =  X_W_dot + self.params['b']
    ...
    def backward(self, X, grad):
        ...
        self.gradient['b'] = np.dot(b_grad,grad)
        ...
        '''
        X_T = X.T
        self.gradient['W'] = np.dot(X_T,grad)
        #self.gradient['W'] = X_T.dot(grad)
        n,d = X.shape
        b_grad = np.ones((1,n))
        self.gradient['b'] = b_grad.dot( grad)
        #self.gradient['b'] = np.dot(np.ones((1, n)), grad)
        W_T = self.params['W'].T
        backward_output= grad.dot(W_T)
...
class tanh:
    ...
    def backward(self, X, grad):
        ...
        ####################################################################################################
        interm_res1 = (np.tanh(X)**2)
        interm_res2 = 1-interm_res1          
        backward_output = grad * ( interm_res2)
        ...
        if hasattr(module, 'params'):
            for key, _ in module.params.items():
                ...
                if _alpha > 0.0:
                    ...
                    #################################################################################
                    g_learn_prod = _learning_rate * g
                    alpha_prod = _alpha * momentum[module_name + '_' + key]
                    #momentum[module_name + '_' + key] = _alpha * momentum[module_name + '_' + key] - _learning_rate * g
                    momentum[module_name + '_' + key] = alpha_prod - g_learn_prod
...
def main(main_params, optimization_type="minibatch_sgd"):
    ...
    for t in range(num_epoch):
        ...
        for i in range(int(np.floor(N_train / minibatch_size))):
            ...
            backward_1 = model['L2'].backward(d1, grad_a2)
            backward_2 = model['drop1'].backward(h1, backward_1)
            grad_a1 = model['nonlinear1'].backward(a1, backward_2)
        ...
        for i in range(int(np.floor(N_train / minibatch_size))):
            ...
            ######################################################################################
            forward_1 = model['L1'].forward(x)
            forward_2 = model['nonlinear1'].forward(forward_1)
            forward_3 = model['drop1'].forward(forward_2, is_train = False)
            a2 = model['L2'].forward(forward_3)
            ######################################################################################
            # NOTE: DO NOT MODIFY CODE BELOW THIS, until next TODO
            ######################################################################################
        ...
        for i in range(int(np.floor(N_val / minibatch_size))):
            ...
            # Make sure to keep train as False
            ######################################################################################
            neural_network1 = model['L1'].forward(x)
            nn_hidden1 = model['nonlinear1'].forward(neural_network1)
            nndrop1 = model['drop1'].forward(nn_hidden1, is_train = False)
            a2 = model['L2'].forward(nndrop1)
